    ...
    $.cbFlyNav.prototype = {
      ...
      _initEvents : function() {
        ...
        self.$trigger.on('click.cbFlyNav', function(e) {
          e.stopPropagation();
          if ( !self.open ) {
          	  _HIHI_CLICKED = !_HIHI_CLICKED;
          	  
            //if ( !self.copied ) {
              //self._copyNav();
              var divX = document.getElementById('ahihi');
              if (divX) {
	              ...
	              div.id = 'ahihi';
	              var html = '';
	              var arr = $('ul.nav.navbar-nav a');
	              var n = arr.length;
	              for (var i = 0; i < n; i++) {
	              	  html += arr.get(i).outerHTML + '<br />';
	              }
	              div.innerHTML = '<div style="width: 100%;position: absolute; top: 45px; padding: 10px; background-color:#5D98A7">' +html+ '</div>';
	              //document.body.appendChild(div);
	              div.innerHTML = '<div style="width: 100%;padding: 10px; background-color:#5D98A7">' +html+ '</div>';
	              jQuery(div).insertAfter('#header-wrap');
